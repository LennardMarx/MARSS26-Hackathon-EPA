    ...
    public class BxReply
    {
        ...
        public static List<Tool> Parse(BinaryPacket packet, int replyOption)
        {
            ...
            for (int i = 0; i < componentCount; i++)
            {
                ...
                // Parse Transform Data if the reply option is enabled
                if ((replyOption & (int)BxReplyOptionFlags.TransformData) != 0)
                {
                    // Transform data only exists if the status is valid
                    ...
                    // Port status and frame number exist if the status is valid or missing.
                    if (component.handleStatus == HandleStatus.Missing || component.handleStatus == HandleStatus.Valid)
                    {
