    ...
    public abstract class Capi
    {
        ...
        public void Listen()
        {
            ...
            // Only read while connected.
                ...
                {
                    // ToDo: Maybe this exception is actually causing us not being able to join the thread sucessfully
                    // somtimes. As this happens on shutdown only, it may be an okay option to just not create any
                    // exceptions and just ignore any pending commands.
        ...
        //AW: Start of ChatGPTs integration of the TX command from the C++ api into the C# api
        /// <summary>
        /// Send the TX command to retrieve tracking data.
        /// </summary>
        /// <param name="options">Options for the TX command, represented as a 16-bit unsigned integer.</param>
        /// <returns>The raw string response from the system.</returns>
        public string GetTrackingDataTX(int options = (int)BxReplyOptionFlags.TransformData | (int)BxReplyOptionFlags.AllTransforms) {
            // Construct the TX command
            string command = $"TX {IntToHexString(options, 4)}";

            // Send the command and get the response
            var cmd = Send(command);
            
            // Ensure the command received a valid response
            if(!cmd.HasValidResponse) {
                throw new InvalidOperationException("Failed to receive a valid response for TX command.");
            }

            // Extract and return the raw response data
            return ((AsciiPacket)cmd.Response).Data;
        }

        /// <summary>
        /// Converts an integer to a hexadecimal string with zero padding.
        /// </summary>
        /// <param name="value">The integer value to convert.</param>
        /// <param name="width">The width of the resulting hexadecimal string.</param>
        /// <returns>A zero-padded hexadecimal string.</returns>
        private string IntToHexString(int value, int width) {
            return value.ToString($"X{width}").PadLeft(width, '0');
        }

        public int ConvertHexFrameNumberToInt(string hexString) {
            if(string.IsNullOrWhiteSpace(hexString) || hexString.Length != 8) {
                throw new ArgumentException("Frame number must be an 8-character hexadecimal string.");
            }
        ...
        /// <summary>
        /// Parses the raw response from a TX command into structured tool tracking data.
        /// </summary>
        /// <param name="rawResponse">The raw string response from the TX command.</param>
        /// <returns>A list of parsed tool tracking data objects.</returns>
        public List<ToolTrackingData> ParseTXResponse(string rawResponse) {
            //UnityEngine.Debug.Log(rawResponse);
            if(string.IsNullOrWhiteSpace(rawResponse)) {
                throw new ArgumentException("The TX response cannot be null or empty.");
            }

            List<ToolTrackingData> toolDataList = new List<ToolTrackingData>();

            try {
                // Split the response into lines
                string[] lines = rawResponse.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
                //UnityEngine.Debug.Log(lines.Length);
                // Extract the number of tools from the first line
                int numberOfHandles = int.Parse(lines[0].Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
                lines[0] = lines[0].Substring(2, lines[0].Length - 2); //getting rid of the first 2 characters, as this is the number of handles
                //UnityEngine.Debug.Log(numberOfHandles);
                for(int i = 0; i < numberOfHandles; i++) {
                    string line = lines[i];
                    // UnityEngine.Debug.Log(line.Length);
                    //UnityEngine.Debug.Log(line);
                    ToolTrackingData toolData = new ToolTrackingData();

                    // Parse tool handle
                    toolData.ToolHandle = int.Parse(line.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);

                    // Check for "MISSING" or "DISABLED"
                    if(line.Contains("MISSING")) {
                        toolData.IsValid = false;
                        toolData.Status = "MISSING";
                        toolData.PortStatus = line.Substring(7, 8); // Port Status
                        toolData.FrameNumber = ConvertHexFrameNumberToInt(line.Substring(15, 8)); // Frame Number
                    }
                    else if(line.Contains("DISABLED")) {
                        toolData.IsValid = false;
                        toolData.Status = "DISABLED";
                    }
                    else {
                        // Parse quaternion (Q0, Qx, Qy, Qz)
                        toolData.Quaternion = new Quaternion(
                            float.Parse(line.Substring(2, 6)) / 10000f,
                            float.Parse(line.Substring(8, 6)) / 10000f,
                            float.Parse(line.Substring(14, 6)) / 10000f,
                            float.Parse(line.Substring(20, 6)) / 10000f
                        );

                        // Parse translation (Tx, Ty, Tz)
                        toolData.Position = new Vector3(
                            float.Parse(line.Substring(26, 7)) / 100f,
                            float.Parse(line.Substring(33, 7)) / 100f,
                            float.Parse(line.Substring(40, 7)) / 100f
                        );

                        // Parse error
                        toolData.TrackingError = float.Parse(line.Substring(47, 6)) / 10000f;

                        // Parse port status and frame number
                        toolData.PortStatus = line.Substring(53, 8);
                        toolData.FrameNumber = ConvertHexFrameNumberToInt(line.Substring(61, 8));

                        toolData.IsValid = true;
                        toolData.Status = "VALID";
                    }

                    toolDataList.Add(toolData);
                }

                // Parse system status (last line)
                string systemStatusLine = lines[lines.Length - 1];
                toolDataList.ForEach(tool => tool.SystemStatus = systemStatusLine.Substring(0, 4));
            }
            catch(Exception ex) {
                UnityEngine.Debug.LogError(ex.Message);
                _logger?.Invoke($"Error parsing TX response: {ex.Message}");
            }
        ...
        /// <summary>
        /// Represents structured tracking data for a tool.
        /// </summary>
        public class ToolTrackingData {
            /// <summary>
            /// Tool handle ID.
            /// </summary>
            public int ToolHandle { get; set; }

            /// <summary>
            /// Indicates whether the tool's transformation data is valid.
            /// </summary>
            public bool IsValid { get; set; }

            /// <summary>
            /// Status of the tool (e.g., VALID, MISSING, DISABLED).
            /// </summary>
            public string Status { get; set; }

            /// <summary>
            /// The tool's rotation as a quaternion.
            /// </summary>
            public Quaternion Quaternion { get; set; }

            /// <summary>
            /// The tool's position as a 3D vector.
            /// </summary>
            public Vector3 Position { get; set; }

            /// <summary>
            /// Tracking error associated with the tool's position.
            /// </summary>
            public float TrackingError { get; set; }

            /// <summary>
            /// Port status as a hexadecimal string.
            /// </summary>
            ...
            /// <summary>
            /// System status as a hexadecimal string.
            /// </summary>
            public string SystemStatus { get; set; }

            public override string ToString() {
                if(IsValid) {
                    return $"ToolHandle: {ToolHandle}, Status: {Status}, Position: {Position}, Rotation: {Quaternion}, TrackingError: {TrackingError}, PortStatus: {PortStatus}, FrameNumber: {FrameNumber}, SystemStatus: {SystemStatus}";
                }
                else {
                    return $"ToolHandle: {ToolHandle}, Status: {Status}, PortStatus: {PortStatus}, FrameNumber: {FrameNumber}";
                }
            }
        }
        //AW: End of ChatGPTs integration of the TX command from the C++ api into the C# api
