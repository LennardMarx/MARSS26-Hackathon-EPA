...
namespace NDI.CapiSample.Protocol.GBF
{
    ...
    /// <summary>
    /// Component containing tool 3D marker error data.
    /// </summary>
    class GbfData3DError : GbfComponent
    {
        /// <summary>
        /// List of marker errors for each tool.
        /// 
        /// Dictionary Key: Tool Handle
        /// </summary>
        public Dictionary<int, List<double?>> errors = new Dictionary<int, List<double?>>();

        /// <summary>
        /// Parse the Marker error component from the stream.
        /// </summary>
        /// <param name="reader">Stream to read from.</param>
        /// <param name="numTools">Number of tools containing marker errors in this component.</param>
        public GbfData3DError(Stream reader, int numTools)
        {
            // Markers are grouped by tools
            for (int i = 0; i < numTools; i++)
            {
                // Parse the tool information
                int toolHandle = BytePacker.UnpackInt(reader, 2);
                int nErrors = BytePacker.UnpackInt(reader, 2);

                // Tools contain markers
                errors[toolHandle] = new List<double?>();
                for (int j = 0; j < nErrors; j++)
                {
                    int index = BytePacker.UnpackInt(reader, 2);
                    float error = BytePacker.UnpackFloat(reader);

                    while (errors[toolHandle].Count <= index )
                    {
                        errors[toolHandle].Add(null);
                    }
                    errors[toolHandle][index] = (double)error;
                }
            }
        }
        ...
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            // Component Info
            sb.AppendFormat("----GbfData3DError \n {0}", base.ToString());

            foreach (var tool in errors)
            {
                // Tool Info
                sb.AppendFormat("toolHandleReference={0:X4}\n", tool.Key);
                sb.AppendFormat("numberOfErrors={0:X4}\n", tool.Value.Count);

                for (int i = 0; i < tool.Value.Count; i++)
                {
                    // Marker Info
                    sb.AppendFormat("--Data3DError:");
                    sb.AppendFormat("error[{0:X4}] = {1}\n", i, tool.Value[i] );
                }
            }
