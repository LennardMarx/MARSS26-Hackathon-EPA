        ...
        public List<Tool> GetToolList()
        {
            ...
            foreach (var frame in data)
            {
                foreach(var component in frame.data.components)
                {
                    ...
                    else if(component.type == ComponentType.Data3DError)
                    {
                        GbfData3DError data = (GbfData3DError)component;
                        foreach(var tool in data.errors)
                        {
                            ...
                            {
                                if (tool.Value[i] != null)
                                {
                                    tools[tool.Key].markers[i].error = tool.Value[i].Value;
                                }
                            }
                        }
                    }
